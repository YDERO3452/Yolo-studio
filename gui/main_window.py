...
class MainWindow(QMainWindow):
    ...
    def _create_annotation_control_bar(self) -> QWidget:
        header = QWidget()
        ...
        mode_hint = QLabel("模式: 矩形框 | A/D 切图 | R/P/O/V 工具")
        mode_hint.setObjectName("ModeHint")
        mode_hint.setMinimumWidth(210)
        mode_hint.setMaximumWidth(320)
        mode_hint.setSizePolicy(QSizePolicy.Policy.Maximum, QSizePolicy.Policy.Fixed)
        top_row.addWidget(mode_hint)

        top_row.addWidget(QLabel("模型:"))
        self.yolo_model_combo = QComboBox()
        self.yolo_model_combo.setEditable(True)
        self.yolo_model_combo.setMinimumWidth(120)
        self.yolo_model_combo.setMaximumWidth(420)
        self.yolo_model_combo.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
        self._populate_yolo_models()
        top_row.addWidget(self.yolo_model_combo, stretch=1)
        ...
        top_row.addWidget(self.yolo_status_label)
        ...
        auto_row.addWidget(self.yolo_replace_check)

        self.yolo_model_class_check = QCheckBox("模型类别(中文)")
        self.yolo_model_class_check.setChecked(True)
        self.yolo_model_class_check.setToolTip("开启时自动翻译模型类别名为中文。")
        ...
        self.yolo_current_btn.setMinimumWidth(82)
        self.yolo_current_btn.clicked.connect(self._run_yolo_auto_label_current)
        auto_row.addWidget(self.yolo_current_btn)

        self.yolo_all_btn = QPushButton("标注全部")
        self.yolo_all_btn.setObjectName("PrimaryButton")
        self.yolo_all_btn.setMinimumWidth(82)
        self.yolo_all_btn.clicked.connect(self._run_yolo_auto_label_all)
        auto_row.addWidget(self.yolo_all_btn)

        self.yolo_progress_bar = QProgressBar()
        self.yolo_progress_bar.setRange(0, 100)
        self.yolo_progress_bar.setValue(0)
        self.yolo_progress_bar.setFixedWidth(90)
        self.yolo_progress_bar.setFixedHeight(22)
        auto_row.addWidget(self.yolo_progress_bar)

        self.class_quick_group = QButtonGroup(self)
        self.class_quick_group.setExclusive(True)
        self.class_quick_buttons: list[QPushButton] = []
        self.class_quick_layout = QHBoxLayout()
        self.class_quick_layout.setContentsMargins(0, 0, 0, 0)
        self.class_quick_layout.setSpacing(4)
        auto_row.addLayout(self.class_quick_layout)
    ...
    def _create_inspector(self) -> QWidget:
        ...
        labels_title.setObjectName("PanelTitle")
        labels_header.addWidget(labels_title)
        labels_header.addStretch()
        self.dirty_badge = StatusPill("已保存", "success")
        labels_header.addWidget(self.dirty_badge)
        ...
        self.class_panel.setMaximumHeight(120)
        layout.addWidget(self.class_panel)

        # Objects section
        layout.addWidget(SectionTitle("Objects"))
        self.annot_list = AnnotationListWidget()
        self.annot_list.setMinimumHeight(80)
        layout.addWidget(self.annot_list, stretch=1)

        object_actions = QHBoxLayout()
        object_actions.setSpacing(3)
        ...
        # File queue section
        layout.addWidget(SectionTitle("文件队列"))
        self.file_search = QLineEdit()
        self.file_search.setPlaceholderText("搜索...")
        self.file_search.setFixedHeight(22)
    ...
    def _create_yolo_auto_label_card(self) -> QFrame:
        card = Card("YOLO 自动标注")
        self.yolo_auto_label_card = card

        card.layout.addWidget(QLabel("模型"))
        self.yolo_model_combo = QComboBox()
        self.yolo_model_combo.setEditable(True)
        self.yolo_model_combo.setMinimumWidth(0)
        self._populate_yolo_models()
        card.layout.addWidget(self.yolo_model_combo)
        ...
        self.yolo_load_btn = QPushButton("加载")
        self.yolo_load_btn.clicked.connect(self._load_yolo_model)
        model_btn_row.addWidget(self.yolo_load_btn)
        card.layout.addLayout(model_btn_row)

        self.yolo_status_label = QLabel("未加载 YOLO 模型")
        self.yolo_status_label.setObjectName("MutedText")
        self.yolo_status_label.setWordWrap(True)
        card.layout.addWidget(self.yolo_status_label)
        ...
        count_row.addWidget(QLabel("最大数"))
        self.yolo_max_det_spin = QSpinBox()
        self.yolo_max_det_spin.setRange(1, 3000)
        self.yolo_max_det_spin.setValue(int(self.config_manager.get("inference", "max_det", 300)))
        count_row.addWidget(self.yolo_max_det_spin)
        card.layout.addLayout(count_row)

        option_row = QHBoxLayout()
        self.yolo_replace_check = QCheckBox("覆盖旧标注")
        self.yolo_replace_check.setChecked(False)
        option_row.addWidget(self.yolo_replace_check)
        option_row.addStretch()
        card.layout.addLayout(option_row)

        self.yolo_model_class_check = QCheckBox("使用模型类别名(自动翻译中文)")
        self.yolo_model_class_check.setChecked(True)
        self.yolo_model_class_check.setToolTip("开启时，自动标注会将模型类别名（如 person）翻译为中文（如 人）并添加到项目类别表。\n关闭时，自动标注框会归到当前选中的类别。")
        model_class_row = QHBoxLayout()
        model_class_row.addWidget(self.yolo_model_class_check)
        model_class_row.addStretch()
        self.yolo_name_map_btn = QPushButton("映射表")
        self.yolo_name_map_btn.setFixedWidth(56)
        self.yolo_name_map_btn.setToolTip("编辑模型类别名 → 项目类别名的映射表")
        self.yolo_name_map_btn.clicked.connect(self._show_class_name_map_dialog)
        model_class_row.addWidget(self.yolo_name_map_btn)
        card.layout.addLayout(model_class_row)
        ...
        self.yolo_progress_bar.setRange(0, 100)
        self.yolo_progress_bar.setValue(0)
        card.layout.addWidget(self.yolo_progress_bar)
        return card
